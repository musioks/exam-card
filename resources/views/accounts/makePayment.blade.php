...
<section id="configuration">
        ...
        <div class="col-xs-12">
            <div class="card">
                <div class="card-header bg-teal bg-accent-4">
                    <h4 class="card-title text-white">Make Payment</h4>
    ...
    <p class="card-text"><a  href="{{url('/payments/allPayments')}}" class="btn btn-info">
        <i class="icon-undo2"> </i> Go Back</a></p>
                        <hr>
       <form  method="post" action="{{url('/payments/makePayment')}}">
          {{csrf_field()}}
          <div class="box-body">
           <div class="row">
          <div class="col-md-12">
          ...
          <div class="row">
            <div class="col-md-4">
                <div class="form-group">
                    <label for="payee_name">Payee Name<i style="color: red;">*</i></label>
            ...
            <div class="col-md-4">
              <div class="form-group">
                  ...
                  @if($errors->has('payee_name'))
                     <span class="danger text-muted">{{ $errors->first('address') }}</span>
                  @endif
              </div>
          </div>
            <div class="col-md-4">
          ...
          <div class="row">
           ...
           <!-- /.form-group -->
              ...
              <div class="row">
              <div class="col-md-6">
               <div class="form-group {{$errors->has('v_id') ? 'has-error':''}}">
                   <label>Votehead<i style="color: red;">*</i></label>
                   <select name="votehead_id" class="form-control">
                   <option>-------Choose votehead----</option>
                   @foreach($votes as $votes)
                   <option value="{{$votes->id}}"> {{$votes->name}}</option>
                   @endforeach
                   </select>
                     @if($errors->has('votehead_id'))
                      <span class="danger text-muted">{{ $errors->first('v_id') }}</span>
                   @endif
               </div>
               </div>
              <div class="col-md-6">
                  <!-- /.form-group -->
                     ...
                     <span class="danger text-muted">{{ $errors->first('description') }}</span>
                  @endif
                     </div>
              </div>
          ...
          <div class="row">
               <div class="col-md-6 col-sm-offset-6">
                   ...
                   <div class="form-group">
                       <input type="submit" name="submit" class="btn btn-success" value="Record Payment">
                   </div>
               </div> 
          </div>         
